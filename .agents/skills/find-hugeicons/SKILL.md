...
```bash
jar -tf <jar_path> | grep -i "<keyword>" | grep "stroke/.*Kt.class" | sed 's|com/orchords/hugeicons/stroke/||;s|Kt.class||'
```

Or as a single pipeline:

```bash
jar -tf $(find ~/.gradle/caches -path "*hugeicons-compose*/jars/classes.jar" | head -1) | grep -i "<keyword>" | grep "stroke/.*Kt.class" | sed 's|com/orchords/hugeicons/stroke/||;s|Kt.class||'
```
...
import com.orchords.hugeicons.stroke.AiMagic
